...
class ChatAssistantController extends Controller
{
    public function reply(Request $request): JsonResponse
    {
        ...
        // 0. Numbered menu shortcuts (user replies with "1", "2", "3", "4", "5")
        if (preg_match('/^\s*[1-5]\s*$/', $message)) {
            $pick = (int) trim($message);
            return match ($pick) {
                1 => $this->replyRooms(),
                2 => response()->json([
                ...
                default => response()->json([
                    'reply' => "Maaf, pilihan yang kamu masukkan tidak tersedia. Silakan ketik angka **1** sampai **5**, atau langsung tanyakan yang kamu butuhkan ya!"
                ]),
            };
        }
        ...
        if (Str::contains($lower, ['halo', 'hai', 'hello', 'hi', 'pagi', 'siang', 'malam', 'sore', 'assalamualaikum', 'oi', 'bro', 'min'])) {
            return response()->json([
                'reply' => "Halo! Selamat datang di **RoomEase AI Customer Service** 🤖🌿\n\nAda yang bisa saya bantu hari ini? Ketik angka atau langsung tanyakan:\n"
        ...
        // Default Fallback
        return response()->json([
            'reply' => "Halo! Saya EaseBot, asisten virtual RoomEase 🤖.\n\nSaya bisa membantu kamu mengecek **ketersediaan kamar**, **harga kamar**, **fasilitas hotel**, atau **status booking** (dengan kode reservasi kamu).\n\nKetik angka **1–5** atau langsung tanyakan yang kamu butuhkan ya!"
        ]);
    }

    private function replyRooms(): JsonResponse
    {
